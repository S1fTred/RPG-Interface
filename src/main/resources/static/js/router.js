// router.js — tiny hash router with dynamic params + guard (надёжный редиспатч)
...
export function navigate(patternOrPath, params = null, query = null) {
    ...
    if (location.hash !== hash) {
        ...
        // если уже на этом же hash — всё равно отрисуем
        dispatch();
    }
}
...
export function dispatch() {
    ...
    const ctx = { path: pathname, params: match ? extractParams(match.r.keys, match.m) : {}, query, hash: full };
...
function compile(pattern) {
    ...
    const keys = [];
    const rx = pattern.split('/').map(seg => seg.startsWith(':') ? (keys.push(seg.slice(1)), '([^/]+)') : escapeRe(seg)).join('/');
...
function buildPath(patternOrPath, params, query) {
    ...
    if (params && /:([A-Za-z0-9_]+)/.test(path)) path = path.replace(/:([A-Za-z0-9_]+)/g, (_, k) => encodeURIComponent(params[k] ?? ''));
